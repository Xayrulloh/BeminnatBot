...
scene.step(async (ctx) => {
  const buttons = inlineKFunction(2, [{ view: "📍 Joyshuvlar ro'yxati", text: "get address" }, { view: "➕ Joylashuv qo'shish", text: "add address" }, { view: "🗑 Joylashuv o'chirish", text: "delete address" }])
...
scene.wait('crud').on('callback_query:data', async (ctx) => {
  ...
  if (inputData === 'add address') {
  ...
  } else if (inputData === 'get address') { // get
    ...
    const buttons = inlineKFunction(2, addresses.map(address => {
...
scene.wait('create_delete').on(['callback_query:data', 'message:text'], async (ctx) => {
  ...
  // checking
  if ((ctx.session.command === 'add address' && !textData && inlineData) || (ctx.session.command === 'add address' && textData && ctx.session.locationNames.includes(textData))) {
  ...
  if (ctx.session.command === 'delete address') {
    ...
    const keyboard = new Keyboard()
      .requestLocation("📍 Yangi joylashuv")
      .resized()
      .oneTime();
